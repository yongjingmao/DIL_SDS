...
from DIL_dataset import Dataset
...
class exp(object):
    ...
    def visualize_single_batch(self):
        ...
        input_batch = batch_data['input_batch']
        nonhole_batch = img_batch * ((mask_batch==0).astype(np.float32))
        
        
        if self.cfg['batch_size'] == 1:
            plot_image_grid(np.concatenate((nonhole_batch, input_batch[:, [0]]), 0), 2, padding=3, factor=10)
        else:
            plot_image_grid(np.concatenate((nonhole_batch, input_batch[:, [0]]), 0), self.cfg['batch_size'], padding=3, factor=15)
    ...
    def optimize_params(self, batch_data):
        ...
        if self.cfg['use_perceptual']:
            ...
            if self.cfg['output_channel_img'] == 1:
                # VGG16 requires RBG image as input
                # For grayscale image, repeat the channel three times to make it RGB
                out_img_RGB = out_img_tensor.expand(self.cfg['batch_size'], 3, self.cfg['frame_size'][0], self.cfg['frame_size'][1])
                img_RGB = img_tensor.expand(self.cfg['batch_size'], 3, self.cfg['frame_size'][0], self.cfg['frame_size'][1])
            elif self.cfg['output_channel_img'] > 3:
                # For image with more bands, select the RGB channel
                out_img_RGB = out_img_tensor[:, [2,1,0], ...]
                img_RGB = img_tensor[:, [2,1,0], ...]
                
                
            feature_src = self.netL(out_img_RGB)
            feature_dst = self.netL(img_RGB)
    ...
    def plot_and_save(self, batch_idx, batch_data, subpath):
        ...
        """
        def save(imgs, subpath):
            res_dir = os.path.join(self.cfg['res_dir'], subpath)
            for i, img in enumerate(imgs):
                if img is None:
                    continue
                fid = imgname_batch[i]
                profile = profile_batch[i]
                batch_path = os.path.join(res_dir, 'batch', '{:03}_{}.png'.format(batch_idx, fid))
                sequence_path_png = os.path.join(res_dir, '{}.png'.format(fid))
                sequence_path_tif = os.path.join(res_dir, '{}.tif'.format(fid))
                if self.cfg['save_batch']:
                    cv2.imwrite(batch_path, np_to_cv2(img))
                else:
                #if batch_idx < batch_data['batch_stride'] or i >= self.cfg['batch_size'] // 2:
                    cv2.imwrite(sequence_path_png, np_to_cv2(img))
                    with rasterio.open(sequence_path_tif, 'w', **profile) as dst:
                        dst.write(img) 
        ...
        imgname_batch = batch_data['imgname_batch']
        profile_batch = batch_data['profile_batch']
        ...
        for i in range(self.cfg['batch_size']):
            for con in contour_batch[i]:
                for pt in con:
                    x, y = pt[0]
                    if out_img_batch[i].shape[0] == 1:
                        out_img_batch[i][:, y, x] = 1
                    else:
        ...
        if not self.cfg['res_dir'] is None and self.cfg['save']:
            save(batch_data['out_img_batch'], subpath)